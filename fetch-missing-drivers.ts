...
async function main() {
  ...
  const yearsToFetch = [2017, 2018, 2019, 2020, 2021, 2022, 2023, 2025];

  console.log('Fetching driver standings for missing years...');
    ...
    try {
      ...
      const standings = data.MRData.StandingsTable.StandingsLists[0]?.DriverStandings || [];
        ...
        for (const standing of standings) {
          ...
          d.f1End = Math.max(d.f1End, year);
          d.f1Start = Math.min(d.f1Start, year);
        ...
        console.error(`Error fetching ${year}:`, (e as Error).message);
      }
  }
